...
# update() can be used to insert an item to the dictionary
d.update(h = 38, i = 42) # h, or i or key values are assumed to be a string format
d.update({3:"test"}) # if you have a key in an integer, you can use this way to add an item
d[4] = "James" # or this way.
...
d2 = {'f': 6, 'g': 11}
d1.update(d2) # d1 + d2
d3 = {**d1, **d2} # A double asterisk ** denotes dictionary unpacking.
...
sorted(d3) # == same as sorted(d3.keys())
...
#                                                                 11 -> 91 to make tie
L = [{'value':10, 'weight':0, 'volume':11}, {'value':19, 'weight':11, 'volume':29}, {'value':12, 'weight':91, 'volume':21}]
...
set1.add('c2')
set2.add('c1')
set2.discard('b2') # remove an item
set1.remove('a2') # remove an item if not present, it raises KeyError
set1.pop() # getting it out of at the end of the set
...
a = set3.update([1, 2, 3]) # only takes an interable
...
newstring = string1.replace('2019', '2020') # not IN PLACE
